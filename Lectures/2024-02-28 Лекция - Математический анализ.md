...
$$\begin{gather}
\underbrace{\int \sqrt{ 4-x^{2} }  \, dx}_{I} = \begin{bmatrix}
u = \sqrt{ x-x^{2} } \\
du = -\frac{x}{\sqrt{ 4-x^{2} }} \\
dv = dx \\
v = x
\end{bmatrix} = \\
= \sqrt{ 4 - x^{2} }\cdot x + \int \frac{x^{2} - 4 + 4}{\sqrt{ 4-x^{2} }} \, dx = \\
= x\sqrt{ 4-x^{2} }-s\underbrace{\int \sqrt{ 4-x^{2} } \, dx}_{I} + 4\int \frac{dx}{\sqrt{ 4 - x^{2} }} 
\end{gather}$$