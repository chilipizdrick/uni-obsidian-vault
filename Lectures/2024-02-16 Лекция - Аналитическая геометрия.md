...
\end{gather}$$
при $|\Delta X|\to 0$
>[!note] Уравнение касательной плоскости: $z=z_{0}+df(x_{0},y_{0})$
[[Дифференциал функции|Дифференциал]] определяет уравнение касательной плоскости
## Сложная функция
...
### Доказательство
$f \in X_{0}\implies f(X_{0}+\Delta X)-f(X_{0})=\sum_{i=1}^{n} \frac{\partial f}{\partial x_{i}}(X_{0})\Delta x_{i}+\bar{o}(|\Delta X|)$
