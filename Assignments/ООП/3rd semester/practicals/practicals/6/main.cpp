...
template <typename T> class Treap {
  protected:
    ...
    // Функция split переписана с использованием итераторов
  ...
  public:
    ...
    // Вызывается ф-ция split, реализованная при помощи итераторов
...
template <typename T> class ImplicitTreap : protected Treap<T> {
  protected:
    ...
    T product(TreapNode<T> *node) {
        if (auto typedNode = dynamic_cast<ImplicitTreapNode<T> *>(node))
            return typedNode->product;
        return T(0);
    }
    ...
    // Функция split в декартовом дереве по неявному ключу теперь также
    // реализована при помощи итераторов
  ...
  public:
    ...
    // Используется split, реализованный с применением итераторов
...
int main() {
    // Инициализация декартового дерева
    Treap<int> t{1, 5, -1, 2, 6, 7};

    // Работа итератора по дереву для вывода элементов в порядке увеличения
    // ключа
    for (auto i : t) {
        cout << i << " ";
    }
    ...
    // Разделение дерева по ключу "4"
    auto [t1, t2] = t.split(4);
    for (auto i : t1) {
        cout << i << " ";
    }
    cout << "\n";
    ...
    // Слияние полученных деревьев дает исходное
    Treap<int> t3 = t1.merge(t2);
    for (auto i : t3) {
        cout << i << " ";
    }
    cout << "\n";

    // Нахождение минимального и максимального значений в дереве
    cout << "Min: " << t3.min() << ", Max: " << t3.max() << "\n";

    cout << "\n";

    // Инициализация декартового дерева по неявному ключу
    ImplicitTreap<double> tt{1, 5, -1, 2, 6, 7};
    for (auto i : tt) {
        cout << i << " ";
    }
    cout << "\n";

    // Разделение дерева по "индексу 4" (индекс - значение групповой ф-ции size)
    auto [tt1, tt2] = tt.splitByIndex(4);
    for (auto i : tt1) {
        cout << i << " ";
    }
    cout << "\n";
    ...
    // Значение групповой функции произведения
    cout << tt.Product() << "\n";
    cout << tt1.Product() << "\n";
    cout << tt2.Product() << "\n";
