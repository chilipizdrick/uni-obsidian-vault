...
SuffixTree::SuffixTree() {
    // Использование уникальных идентификаторов для узлов не необходимо для
    // работы алгоритма, но упрощает задачу визуализации и хранения дерева, а
    // также позволяет эффективнее реализовывать некоторые алгоритмы поиска в
    // построенном дереве.
    internal_node_ID = 0;
...
// Реализация однопроходного алгоритма построения суффиксного дерева.

// Однофазный алгоритм построения суффиксного дерева (Single Phase Algorithm)
...
// Алгоритм одиночного продления суффикса (Single Extension Algorithm)
...
std::string SuffixTree::get_substr(int start_pos, int end_pos) {
    if (start_pos > end_pos)
        return std::string();
    // Алгоритм воспринимает исходную строку как 1-индексированную для удобства
